...
func TestEmitShardAssignmentMetrics(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"newly assigned shard with handover stats": {
			...
			expectedHandoverLatencies:   []*time.Duration{ptr.ToPtr(20 * time.Second)},
		},
		"one assigned shard with no handover stats": {
			previousHeartbeat: &store.HeartbeatState{ReportedShards: map[string]*types.ShardStatusReport{}},
			assignedState: &store.AssignedState{
				AssignedShards:     map[string]*types.ShardAssignment{shardID: {Status: types.AssignmentStatusREADY}},
		...
		"one assigned shard with nil handover stats": {
			previousHeartbeat: &store.HeartbeatState{ReportedShards: map[string]*types.ShardStatusReport{}},
			assignedState: &store.AssignedState{
				AssignedShards:     map[string]*types.ShardAssignment{shardID: {Status: types.AssignmentStatusREADY}},
				LastUpdated:        heartbeatTime.Add(-5 * time.Second),
		...
		"multiple newly assigned shards with handover stats": {
			previousHeartbeat: &store.HeartbeatState{ReportedShards: map[string]*types.ShardStatusReport{}},
			assignedState: &store.AssignedState{
				AssignedShards: map[string]*types.ShardAssignment{
					"shard-1": {Status: types.AssignmentStatusREADY},
			...
			expectedDistributionLatency: ptr.ToPtr(15 * time.Second),
			expectedHandoverLatencies:   []*time.Duration{ptr.ToPtr(30 * time.Second), ptr.ToPtr(25 * time.Second)},
		},
		"multiple newly assigned shards with some handover stats": {
			previousHeartbeat: &store.HeartbeatState{ReportedShards: map[string]*types.ShardStatusReport{}},
			assignedState: &store.AssignedState{
				AssignedShards: map[string]*types.ShardAssignment{
					"shard-1": {Status: types.AssignmentStatusREADY},
			...
			expectedDistributionLatency: ptr.ToPtr(15 * time.Second),
			expectedHandoverLatencies:   []*time.Duration{ptr.ToPtr(25 * time.Second)},
		},
		"multiple newly assigned shards without handover stats": {
			previousHeartbeat: &store.HeartbeatState{ReportedShards: map[string]*types.ShardStatusReport{}},
			assignedState: &store.AssignedState{
				AssignedShards: map[string]*types.ShardAssignment{
					"shard-1": {Status: types.AssignmentStatusREADY},
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.expectedHandoverLatencies != nil {
				...
				for _, expectedLatency := range tc.expectedHandoverLatencies {
					metricsScope.On("RecordHistogramDuration", metrics.ShardDistributorShardHandoverLatency, *expectedLatency).Once()
				}
			}
