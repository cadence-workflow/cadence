...
func TestIwrrNode_Cleanup_PointerEqualityCheck(t *testing.T) {
	...
	// Get reference to the child that will be removed
	root.RLock()
	oldChild := root.children["child"].item
	root.RUnlock()
	...
	if exists {
		...
		// If it's the new child, it should be a different instance
		if container.weight == 2 {
			assert.NotEqual(t, oldChild, container.item, "New child should be different instance")
		}
	}
}
