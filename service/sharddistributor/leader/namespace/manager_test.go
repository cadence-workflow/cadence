...
func TestRunElection(t *testing.T) {
	...
	leaderCh := make(chan bool)
	mockElector.EXPECT().Run(gomock.Any()).Return((<-chan bool)(leaderCh))
	mockProcessor := process.NewMockProcessor(ctrl)
	mockProcessor.EXPECT().Start(gomock.Any()).Return(nil)
	mockProcessor.EXPECT().Stop(gomock.Any()).Return(nil)
	...
	go manager.runElection(ctx, "test-namespace", handler)
