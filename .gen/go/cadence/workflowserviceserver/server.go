...
func New(impl Interface, opts ...thrift.RegisterOption) []transport.Procedure {
	...
	service := thrift.Service{
		...
		Methods: []thrift.Method{
			...
			thrift.Method{
				Name: "GetFailoverEvent",
				HandlerSpec: thrift.HandlerSpec{

					Type:   transport.Unary,
					Unary:  thrift.UnaryHandler(h.GetFailoverEvent),
					NoWire: getfailoverevent_NoWireHandler{impl},
				},
				Signature:    "GetFailoverEvent(GetRequest *shared.GetFailoverEventRequest) (*shared.GetFailoverEventResponse)",
				ThriftModule: cadence.ThriftModule,
			},
	...
	procedures := make([]transport.Procedure, 0, 48)
...
func (h handler) GetFailoverEvent(ctx context.Context, body wire.Value) (thrift.Response, error) {
	var args cadence.WorkflowService_GetFailoverEvent_Args
	if err := args.FromWire(body); err != nil {
		return thrift.Response{}, yarpcerrors.InvalidArgumentErrorf(
			"could not decode Thrift request for service 'WorkflowService' procedure 'GetFailoverEvent': %w", err)
	}

	success, appErr := h.impl.GetFailoverEvent(ctx, args.GetRequest)

	hadError := appErr != nil
	result, err := cadence.WorkflowService_GetFailoverEvent_Helper.WrapResponse(success, appErr)
...
type getfailoverevent_NoWireHandler struct{ impl Interface }

func (h getfailoverevent_NoWireHandler) HandleNoWire(ctx context.Context, nwc *thrift.NoWireCall) (thrift.NoWireResponse, error) {
	var (
		args cadence.WorkflowService_GetFailoverEvent_Args
		rw   stream.ResponseWriter
		err  error
	)
	...
	if err != nil {
		return thrift.NoWireResponse{}, yarpcerrors.InvalidArgumentErrorf(
			"could not decode (via no wire) Thrift request for service 'WorkflowService' procedure 'GetFailoverEvent': %w", err)
	}
