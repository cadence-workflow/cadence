...
func (n *iwrrNode[K, V]) cleanup(now time.Time, ttl time.Duration) bool {
	...
	for _, key := range keysToRemove {
		...
		// 2. Child has no children (no descendants were added since cleanup decision)
		//    If no descendants exist, childrenItemCount must be 0 by definition
		// 3. Child's TTL channel still qualifies for cleanup (no tasks in own channel)
		...
		if container, exists := n.children[key]; exists &&
			container.item == childrenSnapshot[key] {
			container.item.RLock()
			hasNoChildren := len(container.item.children) == 0
			container.item.RUnlock()
			if hasNoChildren && container.item.c.ShouldCleanup(now, ttl) {
