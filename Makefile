...
$(BUILD)/gomod-lint: $(ALL_GOMODS) internal/tools/go.mod | $(BUILD)
	$Q echo "checking for direct submodule dependencies in root go.mod..."
	$(foreach submod_import,$(SUBMOD_IMPORTS),\
		$Q if grep -q "$(submod_import)" go.mod; then \
			echo "ERROR: Root go.mod directly depends on submodule: $(submod_import)" >&2; \
			exit 1; \
		fi; \
		echo "✓ No direct dependency on $(patsubst $(MAIN_MODULE)/%,%,$(submod_import))" $(NEWLINE)\
	)
	$Q touch $@
...
TAG_RELEASE_TAGS = $(VERSION) $(foreach d,$(SUBMOD_DIRS),$(d)/$(VERSION))
